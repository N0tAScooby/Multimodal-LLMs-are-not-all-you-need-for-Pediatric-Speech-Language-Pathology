...
def create_compute_metrics(processor):
    def compute_metrics(pred):
        ...
        jiwer_out = jiwer.process_words(label_str, pred_str)
        cer = jiwer.cer(label_str, pred_str)

        exact_matches = [1.0 if p.strip() == l.strip() else 0.0 for p, l in zip(pred_str, label_str)]
        f1_scores = [calculate_f1(l, p) for l, p in zip(label_str, pred_str)]
...
def train(config=None):
    with wandb.init(config=config, project=WANDB_PROJECT) as run:
        ...
        print(f"Augmentation: Prob={config.aug_prob}, Noise={config.noise_max_amp}, Shift={config.pitch_shift_max}")

        current_seed = random.randint(1000, 9999)
        print(f"--- Run: {run.name} | Seed: {current_seed} ---")
        ...
        args = Seq2SeqTrainingArguments(
            ...
            load_best_model_at_end=True, metric_for_best_model="eval_wip", greater_is_better=True,
        ...
        test_metrics = trainer.evaluate(
            eval_dataset=GLOBAL_DATASET["test"], 
...
sweep_configuration = {
    ...
    "parameters": {
        ...
        "learning_rate": {"value": 0.00019543 },
        "lora_rank": {"values": [128]},
        "dropout": {"values": [0]},
...
#     "parameters": {
#
#         "model_id": { "values": [ "openai/whisper-large-v3"]},
#         "learning_rate": {"value": 0.000038159 },
#         "lora_rank": {"values": [96]},
#         "dropout": {"values": [0.3]},
#
#
...
#     "parameters": {
#
#         "model_id": { "values": [ "openai/whisper-large-v3-turbo"]},
#         "learning_rate": {"value": 0.00030178 },
#         "lora_rank": {"values": [96]},
#         "dropout": {"values": [0]},
#
#
